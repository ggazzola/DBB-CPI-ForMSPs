                    ...
                    threshold = 0.2, nperm = 1, OOB = TRUE, pre1.0_0 = conditional)
{
    ...
    colnames(perror) <- xnames
        for (b in 1:length(object@ensemble)){
            ...
            for(j in unique(varIDs(tree))){ ### GGG varIDs is contained in RandomForest.R
              for (per in 1:nperm){

                if (conditional || pre1.0_0) {
                    tmp <- inp
                    ccl <- create_cond_list(conditional, threshold, xnames[j], input) #GGG list of variables other than j to condition on, based on threshold
                    if (is.null(ccl)) {
                        perm <- sample(which(oob))
                    } else {
                        perm <- conditional_perm(ccl, xnames, input, tree, oob) ###GGG note that permutations are "carried out" and performance "decrease" is also calculated on partitions with one only oob point
                    }
                    tmp@variables[[j]][which(oob)] <- tmp@variables[[j]][perm]
                    p <- .Call("R_predict", tree, tmp, mincriterion, -1L,
                       PACKAGE = "party")
                } else {
                    p <- .Call("R_predict", tree, inp, mincriterion, as.integer(j), ### GGG if conditional==F, the permutation is done by this call (note that this permutation is (randomly) different from that obtained using conditional = T and threshold =1 )
                               PACKAGE = "party")
                }
                ## run through all rows of perror
                perror[(per+(b-1)*nperm), j] <- (error(p, oob) - eoob) # GGG not a proportional increase!
              ...
              for (per in 1:nperm){
                 if (conditional || pre1.0_0) {
                    ...
                    if (is.null(ccl)) {
                        ...
                        perm <- conditional_perm(ccl, xnames, input, tree, oob) ### GGG (cond=ccl, to debug conditional_perm below
                    }
                    tmp@variables[[j]][which(oob)] <- tmp@variables[[j]][perm]
                    p <- pred(tree, tmp, -1L)
                } else {
...
conditional_perm <- function(cond, xnames, input, tree, oob){ ###GGG this gives conditional permutation indices for a given tree
