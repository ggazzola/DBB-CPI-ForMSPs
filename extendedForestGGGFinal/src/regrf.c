...
void regRF(double *x, double *y, int *xdim, int *sampsize,
	   ...
	   int *cat, int *maxcat, int *jprint, int *doProx, int *oobprox,
           ...
           int *maxLevel, int *groupOut, double *permX, int *corr, double *prob) {/*GGGMod*/
    /*************************************************************************
   Input:
  ...
  *************************************************************************/
    ...
    for (j = 0; j < *nTree; ++j) {
		...
		regTree(xb, yb, mdim, *sampsize, lDaughter + idx, rDaughter + idx,
				...
				varUsed, ssnode + idx, decSplit + idx, prob); /* GGGmod*/
    ...
    if (varImp) {
		for (m = 0; m < mdim; ++m) {
			...
			impSD[m] = sqrt( ((impSD[m] / *nTree) -
							  (errimp[m] * errimp[m])) / *nTree ); /*GGG it seems like the standard deviation is not computed with the right formula*/
			if (localImp) {
                for (n = 0; n < nsample; ++n) {
