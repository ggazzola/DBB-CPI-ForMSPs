combine <- function(...) {
   ...
   if (have.imp) {
       ...
       for(i in 1:nforest) {
           ...
           ## exactly right... #GGG I guess not exactly right because the sd ...
           rf$importanceSD <- rf$importanceSD +
               rflist[[i]]$importanceSD^2 * rflist[[i]]$ntree #GGG this takes a sd and converts it into var * n (n=ntree)
       }
