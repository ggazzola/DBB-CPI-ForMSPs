...
"randomForest.default" <-
    function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
             ...
             corr.threshold=1, corr.method="pearson", prob=rep(1, ncol(x)), ...) { #GGGmod
    addclass <- is.null(y)
    ...
    if (is.data.frame(x)) {
        ...
        corr[is.na(corr)] <- 0 			###GGG Looks like no partitioning is done on a categorical variable (its correlation with the variable of interest is given a value of zero), unless corr.threshold = 0
    ...
    if (classRF) {
        ...
        rfout <- .C("classRF",
                    ...
                    DUP=FALSE,
                    PACKAGE="extendedForestGGGFinal")[-1] #GGGmod
        if (keep.forest) {
		...
		rfout <- .C("regRF",
                    ...
                    matrix(double(n * p), n) else double(1),
                    as.integer(abs(corr) > corr.threshold), 
					as.double(prob), #GGGmod
                    DUP=FALSE,
                    PACKAGE="extendedForestGGGFinal")[c(16:28, 36:43, 45:46)] #GGGmod
