...
void regTree(double *x, double *y, int mdim, int nsample, int *lDaughter,
	     ...
	     double *tgini, int *varUsed, double *ssnode, double *improve_node, int *selvar) { /* GGGmod*/
...
void findBestSplit(double *x, int *jdex, double *y, int mdim, int nsample,
		   ...
		   double sumnode, int nodecnt, int *cat, int *selvar) { /* GGGmod*/
    ...
    *decsplit = 0.0;
    critmax = 0.0;
    ubestt = 0.0;
    for (i=0; i < mdim; ++i) mind[i] = i; /* GGGCom mind contains indices 0 through p-1 */
    ...
    for (i = 0; i < mtry; ++i) {
		critvar = 0.0;
		/*GGGrem	j = (int) (unif_rand() * (last+1)); GGGCom int here truncates so (int) 0.5 = 0; */
		/*GGGrem kv = mind[j];						Choose one index between 0 and "last+1" (probability of last+1 is zero, so actually from 0 to last). 
											 last+1 starts from p and decreases at every iteration 
											 kv is the currently chosen variable */
		/*GGGrem swapInt(mind[j], mind[last]); /* GGGcom swap the two integers (one becomes the others and vice versa) */
										/* GGGcom we are putting the index of already sampled variable at the end of mind
										 and since we are sampling from a smaller and smaller left-most part of mind, we never sample the 
										 same variable again */
