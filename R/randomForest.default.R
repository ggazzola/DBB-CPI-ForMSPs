...
"randomForest.default" <-
    function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
             ...
             corr.threshold=1, corr.method="pearson", varToScore =1, ...) { #GGGmod
    addclass <- is.null(y)
    ...
    if (classRF) {
        ...
        rfout <- .C("classRF",
                    ...
                    DUP=FALSE,
                    PACKAGE="extendedForestGGG")[-1] #GGGmod
        if (keep.forest) {
		...
		selvar = 1:p ###GGGAdd
		corrVarToScore = corr[varToScore,]###GGGAdd
		###selvar = selvar[corrVarToScore<1 & abs(corrVarToScore)>=corr.threshold]###GGGAdd No, otherwise the variable of interest will never make any split and will have importance  =0
		selvar = selvar[abs(corrVarToScore)>corr.threshold]###GGGAdd
		selvar = selvar-1 ###GGGAdd (IMPORTANT) in C indices start from 0
		mtry = min(length(selvar), mtry) ###GGGAdd (can't sample more than the variables that are available)
		rfout <- .C("regRF",
                    x,
                    as.double(y),
                    as.integer(c(n, p)),
                    ...
                    matrix(double(n * p), n) else double(1),
                    as.integer(abs(corr) > corr.threshold), 
					as.integer(selvar), #GGGmod
                    DUP=FALSE,
                    PACKAGE="extendedForestGGG")[c(16:28, 36:43, 45:46)] #GGGmod
